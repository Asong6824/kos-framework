...
Content`,
		...
		it("should load .kos.md as project context", async () => {
			writeFileSync(join(cwd, ".kos.md"), "# kos vault");

			const loader = new DefaultResourceLoader({ cwd, agentDir });
			await loader.reload();

			expect(loader.getAgentsFiles().agentsFiles).toContainEqual({
				path: join(cwd, ".kos.md"),
				content: "# kos vault",
			});
		});
		...
		it("should discover Vault Skills under 41_Skills", async () => {
			...
			expect(loader.getSkills().skills.some((skill) => skill.name === "vault-skill")).toBe(true);
		});
