...
export class RpcClient {
	...
	/** Run deterministic kos validation for the whole cwd or selected paths. */
	async validate(paths?: string[]): Promise<ValidationReport> {
		const response = await this.send({ type: "validate", paths });
		return this.getData(response);
	}
