	...
	const handleCommand = async (command: RpcCommand): Promise<RpcResponse | undefined> => {
		...
		switch (command.type) {
			...
			case "validate": {
				...
				const report = command.paths?.length
					? validateChangedFiles(cwd, command.paths)
					: validateVault(cwd);
				return success(id, "validate", report);
			}
			...
			case "configure_model": {
				const input = normalizeModelConfiguration(command);
				const known = session.modelRuntime.getModel(input.provider, input.modelId) !== undefined;
				writeModelConfiguration(getModelsPath(), input, known);
				if (!known || input.baseUrl || input.api) await session.modelRuntime.reloadConfig();
				await session.modelRuntime.saveApiKey(input.provider, input.apiKey);
				const models = await session.modelRuntime.getAvailable();
				const model = models.find((candidate) => candidate.provider === input.provider && candidate.id === input.modelId);
