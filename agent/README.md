...
第一版 MVP 已形成可安装闭环：固定 Pi revision 的三层源码、强制 RPC + YOLO 的 kos-agent、`.kos.md`/Vault Skills 上下文、验证与创建/流转 operations、Obsidian 对话和模型配置、可恢复 session、单文件 host 发布包以及真实模型验收。Web、完整 Eval runtime、inline edit 和高级 session UI 继续后续迭代。
...
- `vault/90_系统/harness/` 是当前 legacy 确定性实现；完成 parity migration 后由 kos-agent 中的 library/CLI 取代。
...
- 默认产品工具：Pi 文件/命令工具，以及从 Pi question examples 改造的 RPC-capable `ask_question`。
