...
- EPUB 自适应排版和窄屏布局。
- 选中文本后点击“添加到摘录”，首次创建关联 Extract，后续继续追加；重复点击不会重复写入。
- 选中文本后点击“添加到 Agent”，把原文、Source、原文件和位置填入 Agent 输入框；需要用户检查并手动发送。

阅读进度保存在插件私有 `data.json`，以 Source 路径为键，不回写 Source 正文。选区摘录由 kos-agent 确定性写入 Extract，并同步 `Source.extract_file`；它不调用模型。当前尚不支持全文搜索、持久划线回显、批注、章节 Summary、扫描 PDF OCR、加密 PDF 或 DRM EPUB。
