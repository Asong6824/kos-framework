...
7. 运行 Harness 检查。
8. 若用户选择本期强化能力，只保存一个有限触发的 `capability_focus`，不携带完整测评报告进入每次对话。
...
```text
42_个人操作画像/<分类>/<画像标题>.md
```

禁止使用：

```text
kos/42_个人操作画像/<分类>/<画像标题>.md
```
...
只有 `applies_to` 命中当前工作流才加载该摘要。`start-day` 默认最多一个建议显式使用能力强化；Goal、硬截止、外部承诺、阻塞和用户选择始终优先。证据不足时形成 draft，不自动替换 active focus。
...
- 不要在每次交互中携带完整盖洛普上下文，也不要让 capability_focus 成为所有回答的固定视角。
- 不要忽略不适用场景和被推翻的旧判断。
...
- active capability_focus 包含 period、behavior、applies_to、max_daily_recommendations，且每日最多一个显式建议。
- Harness 全部通过。
