...
metadata:
  hermes:
    ...
    related_skills: [kos-end-my-day, kos-create-project, kos-manage-task]
    pinned: true
  kos:
    scope: core
    lifecycle: active
...
LLM 完成比较和取舍后，必须把最终结构化建议交给 Harness 原子写回：

```bash
kos-harness save-daily-plan --input '{"date":"YYYY-MM-DD","runId":"...","contextFingerprint":"...","recommendations":[...]}' --format json
```
...
Harness 返回 PlanningContext 和确定性候选后，必须由 LLM 比较 Goal 贡献、解锁价值、时间窗口、延迟代价、Project 风险、当日容量和画像适用边界。不得原样转述规则分数或候选顺序。最多推荐三项：一项主要 Goal 推进，一项硬承诺/解除阻塞/维护，一项可选收尾、探索或能力练习。每项写明 Task、Goal、Project、预计投入、理由和取舍。`off_goal`/`conflicting` 默认不主动推荐；用户已经记录 override 且存在硬承诺时可以纳入，但不重复劝阻。

调用 `save-daily-plan` 把最终 LLM 建议写回同一每日计划的 `recommendations` 管理字段，传回 Harness 生成的 run ID 与 Context fingerprint。不要用通用文件编辑工具直接改这个管理字段。看板只能展示这份最终结构化结果；不能把 Harness 基线直接标为“Agent 建议”。
