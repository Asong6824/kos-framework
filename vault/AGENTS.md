...
Use `.kos.md` as the shared vault context. The runtime rules, templates, and harness scripts are agent-backend agnostic; this file only tells Codex how to operate them.
...
`41_Skills/` stores kos Skills: agent-readable procedures for operating the vault. Some metadata remains Hermes-compatible for users who run Hermes Agent, but Codex should treat the files as kos procedures and follow the object rules plus harness checks.
