...
整个看板使用统一的 12 列、`42px` 固定基础行和 `16px` 间距。点阵时钟、当日任务时刻、年度进度、M5 热点图与六张业务卡全部进入同一个碰撞域；用户在显式编辑模式中可对十张卡按整数格自由拖动和缩放，碰撞项确定性下推。所有卡片复用四边与四角 8 个透明命中区：边缘 hover/active 显示 Nothing 蓝短条，角落显示贴合卡片 `30px` 圆角轮廓的蓝色四分之一圆弧；左边和上边缩放保持相对边固定，缩放时原卡片不淡出也不产生拖动浮层。业务卡尺寸受真实内容行数下限约束，四张工具卡分别受 `6/8/9/6` 行安全下限约束。React 管理单一布局状态和撤销历史，dnd-kit 处理全部卡片的拖动传感器，自定义缩放逻辑统一进行整数格吸附，Framer Motion 只使用 transform 完成机械式避让并尊重 reduced-motion。十张卡的布局统一写入插件 `data.json`；时钟、任务时刻和 M15 每秒刷新，热点图按索引变更重算，指标值本身不持久化。所有一级卡片统一使用 `30px` 圆角。
