...
当前已交付 Reader 的基础阅读和深度阅读首版：Source Markdown、本地 PDF、无 DRM EPUB、PDF 纵向连续滚动、EPUB 跨章节滚动/分页切换、目录/快速翻页、进度恢复、全文搜索、持久划线回显、彩色批注、跳回原文、按稳定 ID 删除、选区预填 Agent，以及当前页/章节和本次阅读 Summary 草稿入口。划线与批注写入关联 Extract，Summary 入口把有界上下文和本次批注预填到全局 Agent，由用户确认发送；直接关联其他知识对象和更完整的阅读成果视图仍是后续目标。kos 不负责电子书商店、版权内容分发、DRM 破解或全功能文献管理。

Reader 与 Agent 使用全局 Agent 侧栏和同一套 kos-agent 会话能力，但其事件、进度和阅读结果由 Reader 视图承载，不经由看板触发或展示。当前已完成查找/创建 Source、打开文档、恢复进度、搜索、批注管理、选区写入 Extract、预填 Agent，以及显式触发 Summary 草稿；自动在章节结束时触发总结不在当前边界：
...
当前 Adapter 契约和实现覆盖打开/关闭、渲染、目录、搜索、位置跳转、进度恢复、选区文本、可恢复锚点、批注回显，以及当前页/章节的有界 Agent Context。PDF 使用页码与归一化矩形，EPUB 使用 CFI range，Markdown 使用文本引用；跨格式业务逻辑仍由统一 Reader 接口承载。
...
当前实现保存 `rects` 或 `cfiRange`、Source、原文件路径、颜色、创建时间和更新时间；Markdown 使用文本引用锚点。锚点不进入 Obsidian 插件 `data.json`，因此插件缓存丢失后仍可从 Extract 重建批注。
...
本轮选型结果：PDF 已使用 Obsidian 公开 `loadPdfJs()`/PDF.js 能力并隔离 viewer 细节；EPUB 已直接使用固定版本的 epub.js。当前没有移植 EPUB Marginalia 或其他社区插件源码，因此不产生对应的源码归属项；后续如需移植其他模块，必须先补充 `THIRD_PARTY_NOTICES`、上游 revision、修改范围和许可证文本。Progress 由 kos 插件私有存储持有；Annotation 与 Extract 使用 kos-agent 确定性 operation 写入 Markdown，阅读 Summary 复用全局 Agent 与 `kos-summarize` Skill。
...
└── React Root
    └── ReaderApp
        ├── Toolbar / TOC（已实现）
        ├── Search / Annotation / Summary Panel（已实现）
        ├── AdapterHost
...
- 当前用纯 TS 单测验证路径、进度、搜索、上下文、批注存储、重建、去重和删除合同；独立临时 Vault 的真实 Obsidian E2E 继续验证 PDF 连续滚动/懒加载、EPUB 滚动/分页切换、位置持久化、恢复和销毁。搜索命中跳转、PDF 矩形回显和 EPUB CFI 回显仍需持续扩展真实 E2E。
...
- Markdown、PDF 和 EPUB 文档内搜索与结果跳转。
- 持久划线回显、彩色批注、删除和跳回原文。
- 当前页/章节与本次阅读 Summary 草稿入口。

下一里程碑支持：

- 在 Reader 内展示 Summary 生成进度、最终结果和关联对象。
- 由用户显式选择 Project、Research、Concept 或 Method 关系。
- 搜索命中在原文中的精确强调与上一条/下一条导航。
- 更完整的阅读 session 生命周期和真实 Obsidian E2E。
...
六张已确认原型继续约束各区块内部的信息密度、留白、状态层级和控件形态；一级页面布局以当前响应式 Bento 规格为准，不再要求原型中的纵向全宽顺序。只有 Obsidian 宿主框架、真实数据差异和响应式约束允许其他必要偏差。字体、资产和代码进入插件前仍需独立核对许可证和分发方式。

整个看板使用统一的 12 列、`42px` 固定基础行和 `16px` 间距。点阵时钟、当日任务时刻、当前半年目标、年度进度、M5 热点图与六张业务卡全部进入同一个碰撞域；用户在显式编辑模式中可对十一张卡按整数格自由拖动和缩放，碰撞项确定性下推。所有卡片复用四边与四角 8 个透明命中区：边缘 hover/active 显示 Nothing 蓝短条，角落显示贴合卡片 `30px` 圆角轮廓的蓝色四分之一圆弧；左边和上边缩放保持相对边固定，缩放时原卡片不淡出也不产生拖动浮层。业务卡尺寸受真实内容行数下限约束，五张工具卡分别受 `6/10/7/9/6` 行安全下限约束。点阵时钟只保留主时钟；任务时刻卡复用同级时钟字号并展示本地日期，不再展示周数、年内日序和秒脉冲状态行；目标概览展示当前半年 Goal 的投入占比、状态、健康度、结果证据，以及由关联 Project 结构化结果指标归一化得到的目标进度，无可计算指标时显示未知，不以投入占比或 Task 完成数代替。React 管理单一布局状态和撤销历史，dnd-kit 处理全部卡片的拖动传感器，自定义缩放逻辑统一进行整数格吸附，Framer Motion 只使用 transform 完成机械式避让并尊重 reduced-motion。十一张卡的布局统一写入插件 `data.json`；时钟、任务时刻和 M15 每秒刷新，目标概览与热点图按索引变更重算，指标值本身不持久化。所有一级卡片统一使用 `30px` 圆角。

## 10. 后续讨论

后续在本文继续补充：

- 六区块单页 Bento 栅格的规格、锚点、区块滚动与响应式结构。
- 其他适合由看板触发的 Agent 工作流。
- 各区块的直接 UI 操作与 Agent 操作边界。
- Agent 结果在看板中的统一组件规范。
- 每日 session 与普通对话 session 的生命周期关系。
- Reader 原文件的 Vault 目录、外部路径与设备同步策略。
- 阅读 session、章节摘要和全书摘要之间的生命周期及结果展示。
- 自动章节完成事件是否值得引入，以及对应的 Eval。
