...
本文评审通过前不覆盖 runtime 对象规范。实施时每个 Phase 必须同步修改 Vault 规范与模板、kos-agent Schema/operation/RPC、插件模型与交互、runtime 用户文档和对应 Eval，并在下游同步或发布前运行 `make release-check`。
